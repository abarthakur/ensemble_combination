...
x1_test, y1_test = np.random.multivariate_normal(mean, cov, size1_test/2).T
mean = [1.4, 1.4]
cov = [[0.1, 0], [0, 0.1]]
x11, y11 = np.random.multivariate_normal(mean, cov, size1/2).T
x11_test, y11_test = np.random.multivariate_normal(mean, cov, size1_test/2).T
x1 = np.concatenate((x1,x11))
y1 = np.concatenate((y1,y11))
x1_test = np.concatenate((x1_test,x11_test))
y1_test = np.concatenate((y1_test,y11_test))


mean = [1.4, 0.2]
cov = [[0.1, 0], [0, 0.1]]
x2, y2 = np.random.multivariate_normal(mean, cov, size2/2).T
x2_test, y2_test = np.random.multivariate_normal(mean, cov, size2_test/2).T
mean = [0.2, 1.4]
cov = [[0.1, 0], [0, 0.1]]
x22, y22 = np.random.multivariate_normal(mean, cov, size2/2).T
x22_test, y22_test = np.random.multivariate_normal(mean, cov, size2_test/2).T
...
np.random.shuffle(X)
pandas.DataFrame(X).to_csv("well_separated_nonlinear.trn", header=None, index=False)
...
np.random.shuffle(X)
pandas.DataFrame(X).to_csv("well_separated_nonlinear.tst", header=None, index=False)